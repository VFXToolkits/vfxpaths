...
class EntityBase:
    def _rule_map(self, format_value: str, extend: str = "") -> str:
        ...
        for item in all_field:
            format_value = format_value.replace("{%s}" %item, self.__field_replace(item))
        return format_value
    ...
    def __field_replace(field):
        current_value = getattr(Configuration.entity_config, field, "")
        if current_value:
            return current_value
        map_key_value = Configuration.entity_config.map_key.get(field, "[misse]")
        return map_key_value
    
    def _get_version_info(self, assets_name: str, version: str = "1", num: str = 1):
        ...
        if version_format in assets_name:
            ...
            assets_name = assets_name.replace(version_format, version_name)
...
class Entity(EntityBase):
    ...
    def get_assets_file_name(self, extend: str = "", version: str = "1", num: str = 1) -> str:
        ...
        return self._get_version_info(assets_name, version, num)
    ...
    def get_custom_name(self, rule_name: str = "") -> str:
        ...
        if current_rule_format:
            assets_name = self._rule_map(current_rule_format.get("name"))
            return self._get_version_info(assets_name)
        return ""

    def full_path(self, rule_name: str = "") -> str:
        assets_name = self.get_custom_name(rule_name)
        if not assets_name:
            log.error("assets is none")
            return ""
